  ...
  return (
    ...
    >
      <div className="relative aspect-16/10 overflow-hidden bg-(--color-surface)">
        ...
        <div className="absolute top-4 right-4 grid place-items-center w-9 h-9 rounded-xl glass text-fg opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all">
          <HiArrowRight />
        </div>
      ...
      <div className="p-6 flex-1 flex flex-col">
        <h3 className="font-display text-lg font-semibold text-fg mb-3 group-hover:text-emerald-500 transition-colors">
          {project.name}
        </h3>
        <div className="flex flex-wrap gap-1.5 mt-auto">
          {project.technologies.split(",").map((t) => (
            <span
              key={t}
              className="px-2.5 py-1 text-[11px] font-medium rounded-full bg-fg/5 border border-border text-fg-soft"
            >
              {t.trim()}
            </span>
          ))}
  ...
  return (
    <div className="flex items-end justify-between mb-8 gap-4">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-display text-2xl md:text-3xl font-semibold text-fg tracking-tight">
          {title}
        </h3>
          ...
          <>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="grid place-items-center w-10 h-10 rounded-xl glass text-fg hover:border-emerald-500/40 hover:text-emerald-500 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <HiArrowLeft />
              </button>
              <button
  ...
  return (
    <>
      ...
      <div className="relative">
        {/* Edge fades */}
        <div className="pointer-events-none absolute inset-y-0 left-0 w-12 bg-linear-to-r from-bg to-transparent z-10 opacity-60" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-12 bg-linear-to-l from-bg to-transparent z-10 opacity-60" />
...
export default function Project() {
  return (
    <section
      id="project"
      className="relative py-28 px-6 md:px-12 lg:px-20 overflow-hidden bg-bg"
    >
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-150 h-150 bg-emerald-500/5 blur-[180px] rounded-full pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        >
          ...
          <motion.h2
            variants={fadeUp}
            custom={1}
            className="font-display text-4xl md:text-5xl font-bold tracking-tight text-fg mb-4"
          >
            Selected work
          </motion.h2>
          <motion.p
            variants={fadeUp}
            custom={2}
            className="text-fg-soft text-lg"
          >
        ...
        >
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 px-6 py-3.5 text-sm font-semibold rounded-xl glass text-fg hover:border-emerald-500/40 transition-colors"
          >
            <HiOutlineCode />
            See more on GitHub
          </a>
